...
CHARACTERS = {
    "sophia": {"name": "София", "age": 22, "description": "Соблазнительная блондинка", "personality": "Игривая, дерзкая", "emoji": "👰", "base_prompt": "sophia"},
    "elena": {"name": "Елена", "age": 24, "description": "Страстная брюнетка", "personality": "Интеллигентная, сексуальная", "emoji": "💃", "base_prompt": "elena"},
    "natasha": {"name": "Наташа", "age": 20, "description": "Озорная рыжеволосая", "personality": "Веселая, раскрепощенная", "emoji": "🔥", "base_prompt": "natasha"},
    "victoria": {"name": "Виктория", "age": 25, "description": "Доминантная ведьма", "personality": "Властная, требовательная", "emoji": "👿", "base_prompt": "victoria"},
    "monica": {"name": "Моника (Сюрприз)", "age": 23, "description": "Сюрприз на годовщину ❤️", "personality": "Взволнованная, любящая", "emoji": "🎁", "base_prompt": "monica"},
}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # 2. Передаем чистую, защищенную строку URL напрямую в Telegram
        if image_prompt or is_photo_requested:
            # Если ИИ не вернул промпт, принудительно ставим базовый
            final_prompt = image_prompt if image_prompt else char["base_prompt"]
            
            await update.message.chat.send_action("upload_photo")
            image_url = ai.generate_image_url(final_prompt)
        ...
        await update.message.reply_text("❌ Извини, произошёл технический сбой. Повтори фразу еще раз!")
